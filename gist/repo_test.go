...
func TestParseRepoAddress(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "github url",
			input:         "https://github.com/godot2d/clash-speedtest",
			expectedOwner: "faceair",
			expectedRepo:  "clash-speedtest",
		},
		{
			name:          "github url without scheme",
			input:         "github.com/godot2d/clash-speedtest",
			expectedOwner: "faceair",
			expectedRepo:  "clash-speedtest",
		},
		{
			name:          "git suffix",
			input:         "https://github.com/godot2d/clash-speedtest.git",
			expectedOwner: "faceair",
			expectedRepo:  "clash-speedtest",
		},
		{
...
func TestUpdateRepoFile(t *testing.T) {
	t.Run("update existing file", func(t *testing.T) {
		...
		if err := uploader.UpdateRepoFile("test-token", "https://github.com/godot2d/clash-speedtest", "configs/result.yaml", "main", []byte("payload")); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if requestCount != 2 {
